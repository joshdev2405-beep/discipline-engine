              ...
              <div key={m.mood} className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-4 text-xs font-mono">
                    ...
                    <span className={m.avgR >= 0 ? "text-profit" : "text-loss"}>
                      {m.avgR > 0 ? "+" : ""}{m.avgR}R
                    </span>
                ...
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <motion.div
                    ...
                    transition={{ duration: 0.8, delay: 0.1 * m.mood }}
        ...
        >
          ...
          <div className="mt-4 space-y-4">
            {tagAnalysis.length === 0 && (
              <p className="text-xs font-mono text-muted-foreground">No tag data yet</p>
            )}
            {tagAnalysis.map((t) => (
              <div key={t.tag} className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-4 text-xs font-mono">
                    ...
                    <span className={t.avgR >= 0 ? "text-profit" : "text-loss"}>
                      {t.avgR > 0 ? "+" : ""}{t.avgR}R
                    </span>
                ...
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${t.winRate}%` }}
                    transition={{ duration: 0.8 }}
                    className={`h-full rounded-full ${t.avgR >= 0 ? "bg-primary/60" : "bg-destructive/60"}`}
                  />
                </div>
              </div>
            ))}
        ...
        >
          ...
          <div className="mt-4">
            ...
            {strategyAnalysis.map((s) => (
              <div
                key={s.strategy}
                className="grid grid-cols-[1fr_60px_60px_80px] gap-2 items-center py-2 px-1 border-t border-border text-xs font-mono"
              >
